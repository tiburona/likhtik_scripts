...
class OpenEphysRawIO(BaseRawIO):
        ...
        if len(info["spikes"]) > 0:
            ...
            for spike_filename_seg0 in info["spikes"][0]:
                ...
                if self._sampling_rate is None:
                    self._sampling_rate = spike_info["sampleRate"]
                else:
                    assert self._sampling_rate == spike_info["sampleRate"], "mismatch in spike sampling rate"
    ...
    def _get_spike_slice(self, seg_index, unit_index, t_start, t_stop):
        ...
        ts0 = int(t_start * self._sampling_rate)
        ts1 = int(t_stop * self._sampling_rate)
    ...
    def _rescale_spike_timestamp(self, spike_timestamps, dtype):
        spike_times = spike_timestamps.astype(dtype) / self._sampling_rate
        return spike_times
