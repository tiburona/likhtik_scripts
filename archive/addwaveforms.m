for i = 1:length(single_cell_data)
    animal = single_cell_data(i);
    if ~ismember(animal.animal, {'IG154', 'IG156', 'IG158', 'IG160', ...
            'IG163', 'IG176', 'IG177', 'IG178', 'IG179', 'IG180'})
        continue
    end
    addwaveforms_ks(single_cell_data(i), true)
end

function data=addwaveforms_ks(data,plot_figures)
...
M=memmapfile(fullfile(day_dir,'data_binary.bin'));
M=memmapfile(fullfile(day_dir,'data_binary.bin'),'Format',{'int16',[16 length(M.Data)/16/2],'data'});
...
chMap = readNPY(fullfile(gwfparams.dataDir, 'channel_map.npy'))+1; %KRS edit
noise_chan = noise_chan(chMap); %KRS edit
...
%        plot(reshape(squeeze(cat(3,average_form((cgs==2),n_elec,36:end),NaN*ones(sum(cgs==2),1,3)))',1,[])-(n_elec-1)*200,'k');hold on;
    ...
    for n_elec=1:size(average_form, 2)
        avg = average_form((cgs==2), n_elec, 36:end);
        nanmat = NaN*ones(sum(cgs==2), 1, 3);
        cat_avg_and_nanmat = cat(3, avg, nanmat);
        squeeze_cat = squeeze(cat_avg_and_nanmat)';
        reshape_squeeze = reshape(squeeze_cat, 1, []);
        plot(reshape_squeeze - (n_elec-1)*200, 'k');
        hold on;
    end
end
...
try
    ...
    backgroundLFP=squeeze(nanmedian(average_form(n,in2(variability(1:5)),baseline:end),2)); 
