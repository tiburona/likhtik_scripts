...
process_continuous_data <- function(element_type, data_dir) {
  
  data_file <- file.path(data_dir, paste('proportion_score_continuous', '.csv', sep=""))
  # Read the CSV filed
  data <- read.csv(data_file, row.names = NULL)
...
continuous_unit_data = process_continuous_data('unit', data_dir)
continuous_unit_model = lmer(rate ~ condition*category*period + (1|animal/unit_num), data=continous_unit_data)

tab_model(continuous_unit_model)
